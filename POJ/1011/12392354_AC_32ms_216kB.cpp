...
int DFS(int len, int remains_len, int num)/*作为搜索的棒长len，拼成len长的棒还需的长度remains_len，剩下的未作为拼接的小棒个数num*/
{
	...
	for (int i = 0; i<n; i++)
	{
		...
		if (remains_len >= a[i])
		{
			...
			if (a[i] == remains_len || len == remains_len)
				break;/*a[i]=remains_len或remians_len=len时本来表示即将拼成一根棒，或刚进行新一轮拼接(即if中判断的是主问题性质完全相同的子问题）
				 但经过上面的if判断，子问题不能完成所有任务，那么整体不可能完成任务，不再考虑，搜索失败（剪枝）*/
